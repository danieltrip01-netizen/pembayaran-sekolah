...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex gap-2">
            <a href="{{ route('laporan.pdf', array_filter($filter)) }}" class="btn btn-danger btn-sm" target="_blank">
                <i class="bi bi-file-pdf me-1"></i>Download PDF
            </a>
            <a href="{{ route('laporan.excel', array_filter($filter)) }}" class="btn btn-success btn-sm">
        ...
        <div class="card-body py-3">
            <form method="GET" class="row g-2 align-items-end">
                ...
                <div class="col-md-2">
                    ...
                    <input type="month" name="bulan" value="{{ $filter['bulan'] ?? '' }}"
                        class="form-control form-control-sm" placeholder="Semua bulan">
                </div>
                ...
                <div class="col-md-2 d-flex gap-1">
                    ...
                    <a href="{{ route('laporan.index') }}" class="btn btn-outline-secondary btn-sm" title="Reset filter">
                        <i class="bi bi-x"></i>
                    </a>
                </div>
            </form>
    ...
    <div class="row g-3 mb-3">
        ...
        @if (empty($filter['jenjang']) || $filter['jenjang'] === 'TK')
            <div class="col-6 col-md-3">
                <div class="card border-0 shadow-sm text-center h-100">
                    <div class="card-body py-3">
                        ...
                        <div class="card border-0 shadow-sm" style="border-left:4px solid {{ $js['color'] }} !important">
                            <div class="card-body py-3">
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    ...
                                    <small class="text-muted">{{ $grp->pluck('siswa_id')->unique()->count() }}
                                        siswa</small>
                                </div>
                                ...
                                @if (count($jenjangOptions) > 1 && empty($filter['jenjang']))
                                    <td>
                                        @php
                                            $jKelas =
                                                $pembayaran
                                                    ->filter(fn($p) => ($p->siswa->kelas ?? '-') === $kelas)
                ...
                <tbody>
                    @forelse($pembayaran as $idx => $p)
                        ...
                        <tr>
                            ...
                            <td class="fw-600">
                                <a href="{{ route('pembayaran.show', $p) }}" class="text-decoration-none"
                                    style="color:var(--primary)">
                                    {{ $p->siswa->nama ?? '-' }}
                                </a>
                            </td>
                            {{-- FIX: filter siswaKelas berdasarkan tahun_pelajaran_id yang aktif/dipilih --}}
                            <td>{{ $p->siswa?->siswaKelas->firstWhere('tahun_pelajaran_id', $filter['tahun_pelajaran_id'])?->kelas?->nama ?? '-' }}
                            </td>
                            <td>
                                <span class="badge rounded-pill px-2"
